<template>
  <div class="field-edit-inner" data-type="gallery">
    <div class="field-edit-header" :class="{ 'inner-open' : model.open }" @click="model.open = !model.open">
      <h3 class="mb-0">{{ model.title }} <small :class="{ 'ml-3': model.title && model.title != '' }">GALLERY</small></h3>
    </div>
    <div class="field-edit-body mt2">
      <div class="row align-items-center">
        ...
        <div class="col-12">
          <div class="form-group">
            <label class="form-control-label">Accepted file extensions</label>
            <input type="text" class="form-control" v-model="model.accept"> 
          </div>
        </div>
      </div>
    </div>
...
module.exports = {
  ...
  mounted: function() {
    if (!this.model.location) {
      Vue.set(this.model, 'location', 'sellfino')
    }
  }
}
</script>